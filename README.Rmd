...
We will illustrate the usage and (substandard) performance of this package. To achieve this, we will first load the _prepair_ package. We will also load two other packages for working with spatial vector data.
...
Next, we will obtain a large spatial dataset containing invalid geometries. Specifically, we will download data from Database of Global Administrative Areas (https://gadm.org) that delineates the boundaries of the United States of America (USA).

```{r}
# download boundary for the USA
data_path <- "usa_data.rds"
if (!file.exists(data_path))
  download.file(paste0("https://biogeo.ucdavis.edu/data/gadm3.6/Rsf/",
                       "gadm36_MLT_0_sf.rds", data_path))
...
# plot data
plot(st_geometry(x), main = "raw USA data")

# verify that dataset has invalid geometries
...
# plot repaired data
plot(st_geometry(y), main = "repaired USA data")
```
